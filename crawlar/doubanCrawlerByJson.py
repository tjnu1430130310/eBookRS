    ...
    for x in range(10):
        ...
        try:
            ...
            for y in range(len(reviews)):
                reviewUrl = 'https://read.douban.com' + reviewsSoup.select('a[class="date"]')[y].get('href')
                try:
                    request_review = urllib2.Request(reviewUrl, headers=sendHeaders)

                    reviewHtml = urllib2.urlopen(request_review)
                    reviewSoup = BeautifulSoup(reviewHtml, 'lxml')  # print reviewsSoup


                    user_Name = reviewSoup.select('a[class="review-author url fn"]')[0].text
                    print user_Name
                    # 存入数据库
                    ...
                    rating = reviewSoup.select('meta[itemprop="ratingValue"]')[0].get('content')  # print type(rating)
                    context = reviewSoup.select('div[class="description"]')[0].text  # print context
                    dtstr = reviewSoup.select('span[itemprop="datePublished"]')[0].text + ' ' + reviewSoup.select('span[class="time"]')[0].text  # print dtstr
                    reviewTime = sqlite3.datetime.datetime.strptime(dtstr, '%Y-%m-%d %H:%M:%S')  # print reviewTime

                    # 存入 Reviews 表
                    try:
                        cur.execute(
                            'INSERT OR IGNORE INTO comments_comment(book_id, user_id, username, email, url, score_rating, context, read_count, created_time) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)',
                            (book_id, user_id, user_Name, '1@1.com', '', rating, context, 1, reviewTime))
                        review_id = cur.lastrowid
                        conn.commit()
                        print 'review ' + str(review_id) + ' is got'
                    except:
                        print 'review get failed'
                        # time.sleep(int(format(random.randint(0, 9))))  # 设置一个随机数时间，每爬一个网页可以随机的停一段时间，防止IP被封
                except:
                    print 'URL error, review get failed: ' + reviewUrl
        except:
            print 'URL error, reviews get failed: ' + reviewsUrl
...
# for i in range(11):
for i in range(100):
    url = urlTmep + str(i*20) + '&sort=hot&promotion_only=False&min_price=None&max_price=None&works_type=None'   # 页码是通过get方式获取，同每页在后面都是20的倍数，一共3页
    print url
